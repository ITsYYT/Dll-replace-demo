...
class SCConfigInterface : public CConfigInterface
{
};

class SCConnectionInterface : public CConnectionInterface
{
     ...
     * 如果应用不需要任何回调方法，则可向该方法传递NULL，而不必自定义回调类和对象
     */
    int FUNCTION_CALL_MODE Create(CCallbackInterface *lpCallback);
     ...
     * @return 返回0表示成功，否则表示失败，通过调用GetErrorMsg可以获取详细错误信息
     */
    int FUNCTION_CALL_MODE Connect(unsigned int uiTimeout);

    /**
     * 断开连接
     * @return 返回0表示成功，否则表示失败，通过调用GetErrorMsg可以获取详细错误信息
     */
    int FUNCTION_CALL_MODE Close();
